        ...
        <div className="space-y-2">
          {recentShifts.length > 0 ? (
            recentShifts.map((shift) => (
              <article key={shift.id} className="flex items-start gap-3 rounded-[14px] border border-[#464BD8]/30 bg-[#464BD8]/5 px-4 py-3">
                <Info className="mt-0.5 size-[22px] shrink-0 text-[#464BD8]" />
                <div className="space-y-0.5">
                  <p className="text-[15px] font-medium text-[#1E2330]">
                    Dr. {shift.doctors?.first_name} {shift.doctors?.last_name}
                  </p>
                  <p className="text-[13px] text-slate-500">{shift.doctors?.speciality}</p>
                  <p className="text-sm text-[#464BD8]">
                    {shift.start_date} | {shift.start_time.substring(0, 5)} - {shift.end_time.substring(0, 5)}
                  </p>
                </div>
              </article>
            ))
